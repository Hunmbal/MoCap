...
class Human:
    ...
    def __init__(self, temp) -> None:
        ...
        # # Upper r
        self.r_shoulder = BODYPART("r_shoulder", self.r_elbow,type=temp)
        ...
        for name, values in data.items():
            if hasattr(self, name):
                try:
                    ...
                    body_part = getattr(self, name)  # Type hint optional
    ...
    def __get_data__(self):
        ...
        for part_name in [attr for attr in dir(self) if not attr.startswith('_')]:
            body_part: BODYPART = getattr(self, part_name)
            if isinstance(body_part, BODYPART):
                # return a tuple of (pitch, roll, yaw)
                data_dict[part_name] = (
                    float(body_part.pitch),
                    float(body_part.roll),
                    float(body_part.yaw)
                )
        return data_dict
