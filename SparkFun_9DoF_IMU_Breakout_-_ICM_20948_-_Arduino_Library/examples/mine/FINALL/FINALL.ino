...
#define NUM_SENSORS 4 // Change this to the number of sensors
// Initialize the CS pins and names
...
ICM_20948_SPI myICM[NUM_SENSORS]; // Create an array of ICM_20948_SPI objects
...
void loop() {
    ...
    strr += "sent";
    BT.println(strr);
    delay(20); // Reduced delay to improve loop frequency
}


// format must match the recieving format of DSP
...
void initSensor(int index) {
    ...
    // myICM[index].swReset(); // Reset ICM-20948
    
    bool success = true;
    int tries = 0;
    while (tries < 5) {
        myICM[index].begin(CS_PINS[index].pin, SPI_PORT);
